...
These errors are also known as false negatives (FN) and they occur when we fail to reject the null hypothesis but we should have rejected it.

For our example of screening for HIV in blood donors, a false negative would occur if the screening test returns that the blood sample is HIV negative when the donor in fact does have HIV. Due to the safety risks this blood poses to recipients, we would also want a very small false negative rate.
...
#### What we can do with p-values


#### What we shouldn't do with p-values

* Because p-values depend on the sample size as well as the effect size, it is not appropriate to compare p-values to each other. One being smaller than the other could be due to differences in sample sizes. Especially
